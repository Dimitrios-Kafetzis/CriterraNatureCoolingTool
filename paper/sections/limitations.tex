...
The practical consequence is that a difference of two or three points in an
Opportunity Score between two options carries no meaning. The tool distinguishes
...
interventions.} Street tree planting, shaded pedestrian corridors, and
permeable shaded plazas deliver more human benefit than their air-temperature
figures convey.
...
Three parts of the methodology are declared but not fully specified at this
version, and are collected here so that a reviewer has them in one place:
the input-to-sub-indicator rules for the NbS Suitability and Equity scores
...
(\Cref{sec:confidence}). In addition, the country emission factor table ships
empty (\Cref{sec:ghg}), so greenhouse-gas outputs are reported as
\emph{not calculated} unless a deployment supplies its own factor. Each of
these requires a version bump and a corresponding update to this document when
it is resolved.
