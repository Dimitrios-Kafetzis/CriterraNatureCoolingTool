...
from nature_cooling.engine.config import Suitability, Typology
...
def test_below_minimum_area_flag_message(config, build_input):
    block, _ = suitability.compute(
        build_input(site_area_m2=400, nbs_type="riparian_restoration"),
        _typology(config, "riparian_restoration"),
        config,
    )
    assert block.suitable is False
    assert block.flags[0].message == (
        "site area 400 m2 is below the minimum viable area of 1000 m2 for Riparian restoration"
    )


def test_soil_below_requirement_flags(config, build_input):
    """Urban forest requires moderate soil; limited soil disqualifies."""
    block, _ = suitability.compute(
        build_input(site_area_m2=5000, nbs_type="urban_forest", soil_availability="limited"),
        _typology(config, "urban_forest"),
        config,
    )
...
def test_soil_no_requirement_scores_full(config, build_input):
    ...
    block, _ = suitability.compute(
        build_input(nbs_type="green_roof", soil_availability="none"),
        _typology(config, "green_roof"),
        config,
    )
...
def test_irrigation_below_requirement_flags(config, build_input):
    ...
    block, _ = suitability.compute(
        build_input(nbs_type="green_facade", irrigation_availability="occasional"),
        _typology(config, "green_facade"),
        config,
    )
...
def test_urban_context_match_and_mismatch(config, build_input):
    ...
    mismatched, _ = suitability.compute(
        build_input(land_use="industrial"), _typology(config), config
    )
