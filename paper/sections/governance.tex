...
Methodology Report in the repository, and every configuration file, and is
recorded in every assessment result the engine produces. The version is a
...
Existing assessments are never silently recomputed. A result retains the
version that produced it, and the interface indicates when a newer methodology
version is available, leaving the decision to re-run with the user. Comparisons
between results produced under differing versions raise a warning.
...
\begin{enumerate}
  ...
  unsourced choice, it is a value position rather than a finding, and the
  sensitivity analysis that will quantify its influence has not yet been run.
