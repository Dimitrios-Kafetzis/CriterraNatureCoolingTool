...
area in a temperate climate. The proposed intervention is a \emph{tree avenue}
--- one of the \num{110} catalogue entries --- which inherits the
\emph{street tree canopy} archetype (\Cref{tab:typologies}) and with it an
envelope of \SIrange{0.5}{3.0}{\degreeCelsius}, a base cooling score of 75,
and a \emph{high} evidence rating. Every performance number below is the
archetype's, and the result says so. This is a package of one: the combination
rules of \Cref{sec:packages} apply but have nothing to combine.
\Cref{tab:we-inputs} gives the inputs.
...
\emph{Climate.} The street tree canopy is a \emph{green}-family archetype and
the site is in a \emph{temperate} zone, which \Cref{tab:climate-matrix} rates
\emph{good}, so $f_{\mathrm{climate}} = 1.0$.
...
The inherited street tree canopy archetype has base cooling score $B = 75$ and
envelope $[\tau_{\min}, \tau_{\max}] = [0.5,\, 3.0]$. By \Cref{eq:cps},
...
\qty{450000}{\kilo\watt\hour} is supplied, and the street tree canopy archetype
is flagged as capable of affecting adjacent building loads. By
\Cref{eq:energy-range},
...
\paragraph{Co-benefits.} The street tree canopy archetype carries library
default levels of \emph{medium} biodiversity, \emph{medium} stormwater,
\emph{high} public health, and \emph{high} urban quality. As noted in
...
\qty{100}{\metre\squared} minimum inherited from the archetype
($S_{\mathrm{space}} = 100$); soil availability \emph{limited} exactly meets the
inherited \emph{limited} requirement ($S_{\mathrm{soil}} = 75$); irrigation
\emph{occasional} exactly meets the \emph{occasional} requirement
($S_{\mathrm{water}} = 75$);
...
\emph{high}; the street tree canopy archetype carries \emph{high} evidence
confidence, so no cap applies). The energy block has the relevance confirmation and the demand
...
Temperature reduction      & \qtyrange{0.50}{2.85}{\degreeCelsius} (daytime, pedestrian-level air; on the street tree canopy evidence) & High \\
...
Five properties of the methodology are visible in the arithmetic above, and
each corresponds to a design commitment from \Cref{sec:commitments}.

\textbf{The evidence class is named, not implied.} Every performance value used
above --- the base score, the envelope, the minimum area, the soil and
irrigation requirements, the co-benefit defaults, the evidence rating, and the
building-energy applicability --- belongs to the street tree canopy archetype,
not to the tree avenue entry, and the result says so. A reader can therefore see
at what resolution the claim is made, which is the point of
\Cref{sec:archetypes} and the limitation of \Cref{sec:limit-inheritance}.
