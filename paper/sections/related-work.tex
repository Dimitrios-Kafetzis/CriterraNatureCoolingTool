...
adjustable in configuration, and defended empirically through sensitivity
analysis rather than by appeal to authority. Presenting them as derived would
be the more persuasive move and the dishonest one.
...
to be small. \Cref{sec:sensitivity} specifies that analysis in full. It also
states clearly that at methodology version \methver{} the analysis is a
specified commitment whose results do not yet exist.
...
\begin{figure}[htbp]
\centering
\input{figures/climate-dependence}
...
states; no bar is drawn for Cfa/Cfb, for which a central estimate is reported
without an interval. The dashed line marks \qty{3.0}{\degreeCelsius}: the
