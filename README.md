...
A user describes a site through a short questionnaire — an optional map step that can fill in the site's area, country and climate zone, followed by six steps of questions — selects one or more of 121 NbS typologies (tree avenues, microforests, green roofs, constructed wetlands, food forests, …) from a menu filtered to what actually suits the site. Every entry opens a **detail view** stating what it is, the cited evidence class its cooling numbers inherit — each source with the finding it supports and its full reference, DOI linked — why the catalogue keeps it, how each suitability condition compares with the described site (with the numbers stated), and, where one exists, a photograph of a real implementation in the site's climate zone. The user then receives:
