...
version expands the catalogue from \num{110} entries to \num{121}
(\Cref{sec:supplement}) and moves no aggregation weight, no cooling envelope,
no scoring formula and no golden scenario --- every added entry inherits an
archetype that already existed, so nothing this analysis measures changed. The
analysis was \emph{re-executed rather than assumed}, and every figure below is
identical to the one published at version \prevver{}; the only difference in
the generated file is its version stamp. A release that changes nothing the
analysis measures should be able to demonstrate that rather than assert it.

The figures themselves were regenerated at version 2026.08.04, when the library
