...
Cooling access carries the smaller weight for two stated reasons: it is a
single indicator where the other two are composites, and it is the field most
...
\emph{moderate}, \emph{reliable}\,$\rightarrow$\,good;
\emph{high}\,$\rightarrow$\,excellent. \\
\addlinespace
Scale &
...
\begin{keynote}
\noindent\textbf{Specification gaps at version \methver{}, disclosed.} Three
things about this subsection are incomplete at the current methodology version,
and a reviewer should know which parts of the tool are fully specified and
which are not.

\begin{enumerate}
  \item The weights in \Cref{eq:suitability,eq:cobenefit,eq:equity} are
  declared in \texttt{config/weights.yaml}, but the rules mapping user inputs
  to the individual sub-indicator scores $S_{\mathrm{space}}$,
  $S_{\mathrm{soil}}$, $S_{\mathrm{water}}$, $S_{\mathrm{maint}}$,
  $S_{\mathrm{context}}$ and to the four equity sub-indicators are
  \emph{not} specified in version \methver{}. They are to be specified
  alongside the engine implementation and will require a version bump and an
  update to this document.

  \item The co-benefit weights include a \emph{social inclusion} term, but the
  typology library supplies default levels for only four of the five
  sub-indicators (biodiversity, stormwater, public health, urban quality).
  Absent a user override, social inclusion falls to the neutral default of
  \Cref{sec:unknown} and is itemised as an applied assumption.

  \item The Equity Score of \Cref{eq:equity} is computed and reported as an
  output block with its own confidence rating, but it does \emph{not} enter the
  final aggregation of \Cref{sec:aggregation}. Equity influences the headline
  score through the Vulnerability Score instead. Whether this is the right
  design is a legitimate question for reviewers; it is recorded here so that
...
loads. Otherwise the result carries an explicit status ---
\texttt{not\_applicable}, \texttt{missing\_energy\_demand}, or
\texttt{typology\_not\_applicable} --- and \textbf{never a zero}, which would
...
feasibility. As with the sub-scores of \Cref{sec:composite-subscores}, the
bracket boundaries and the combination rule realising $f$ are not specified in
version \methver{} and are to be fixed alongside the engine implementation.
...
deployment, and defended empirically through the sensitivity analysis of
\Cref{sec:sensitivity} rather than by appeal to authority.
...
those of \Cref{eq:effective}. Both columns sum to unity.}
...
result records the methodology version under which it was produced.
\end{keynote}
