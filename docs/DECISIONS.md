...
## D-049 — The v2.2 design rulings: the deployment tile concession, the map library, and offline navigation (2026-08-02)

v2.1 shipped a map that is honest and offline and, reviewed in a browser, not usable enough to find a real site. v2.2 makes it usable, and it does so by admitting a second, deliberately narrow relaxation of the no-third-party-request rule — this one at the deployment level rather than the per-user level. The intended deployment model is the whole point and is worth stating as three sentences: an operator who runs this tool for their own users — Criterra at criterra.eu, or any institution on its own premises — may configure a map-imagery source for that deployment, and their users then get real imagery without each having to paste a URL. An operator who configures nothing gets exactly what v2.1 shipped: the bundled offline map, and a per-user opt-in. A plain `pip install` therefore still phones out to nobody.

**D-049.1 — The rule's wording changes from "the package never makes a third-party request" to "the package makes no third-party request until a deployer configures one, or a user opts in." This supersedes part of D-047.1.** What D-047.1 conceded was per-user: external tiles existed only if *the user at the keyboard* named a source and switched it on. What is now additionally conceded is that *the operator of a deployment* may make that choice once, for all of that deployment's users, through runtime configuration — the first runtime configuration this application has ever had. What is **not** conceded, and is now machine-checked at runtime rather than only by static inspection (D-049.8): no tile host, no API key and no default source exists anywhere in the repository, the wheel, or the build output; an unconfigured installation makes no third-party request from first launch through a completed assessment; the backend process itself never makes a third-party request whether configured or not (D-049.3); and a restricted-network deployment keeps working exactly as it does today, configured or not (tile failure degrades to the bundled outlines, verified by the runtime gate).

**D-049.2 — Configuration is two environment variables plus a CLI convenience flag, and there is no config file.** `NATURE_COOLING_TILE_URL` (a `{z}/{x}/{y}` template) and `NATURE_COOLING_TILE_ATTRIBUTION` (the credit line that source requires), read once at application creation and served to the frontend through `GET /api/meta`. `nature-cooling serve --tile-url … --tile-attribution …` sets the same two variables before handing off to uvicorn, so a local run with imagery is one command. A config file was considered and rejected: it is a third mechanism for one setting, and it introduces a file-location-and-precedence concept the application otherwise does not have, to serve no deployer that the other two do not already serve — a container deployment sets environment variables (`compose.yaml` carries a commented example), and a local run passes the flag. **The attribution is not optional:** a tile URL configured without its attribution string refuses to start, with an error saying why. That is the structural fix for the attribution defect v2.1 shipped (a user could enable OSM-derived tiles and see no "© OpenStreetMap contributors" anywhere) — the credit travels with the URL in one configuration unit and cannot be omitted, and the map renders it whenever the layer is on. The per-user opt-in form gains the same required field for the same reason.

**D-049.3 — Tiles are requested browser-direct, never proxied through the backend.** The backend has never made an outbound network request and still never does — that sentence survives this release unconditionally, which a proxy would have destroyed. Browser-direct exposes the operator's API key in the tile URL; that is normal for raster tile services and is managed the way every provider documents: restrict the key to the deployment's domain(s), which the hosting page walks through. Proxying was rejected because it buys key secrecy and cache-ability at the cost of new outbound-network code in the backend, the operator's bandwidth, and making the operator's server an interposer of what areas its users look at.

**D-049.4 — The deployer's source is on by default for that deployment's users; a user may switch it off for their visit, and may substitute their own source either way.** A deployment that configures imagery exists to show imagery — an operator who wanted per-user consent theatre would simply not configure a source — so the configured layer renders by default, with its attribution. The user keeps two controls, both per-visit and never persisted (unchanged from v2.1): turn the imagery off (it is their browser and their IP address that talk to the tile host, and the map must remain fully usable without tiles anyway), and name a different source of their own, which then requires its attribution text exactly as the deployer's did. A user-named source replaces the deployed one for that visit only.

**D-049.5 — Leaflet is adopted, superseding D-048.5.** D-048.5's decisive objection was that a map library "would not instantiate under the test environment at all". That was true of the WebGL libraries under consideration and remains true of them; it was tested against Leaflet before this ruling and is **false** for Leaflet, which instantiates completely under the project's jsdom test environment — map, SVG vector renderer, tile layer, attribution control, and synthetic click-to-coordinate conversion all work under vitest with the same `getBoundingClientRect`-style mocks the existing map tests already use. With that objection gone, the comparison is: hand-rolling fractional zoom, cursor-anchored wheel zoom, drag inertia, touch pinch, date-line wrapping and high-DPI tile selection — several hundred lines of the most defect-prone interaction code in frontend engineering — against 42 KB gzipped of BSD-2-Clause code with **zero dependencies** and twelve years of hardening on exactly those interactions. The library ships inside the wheel as part of the frontend bundle; its licence text ships beside the build as the font OFL notices do, it takes a NOTICE entry, and the bundled-content licence gate extends over it. MapLibre GL (BSD-3, 17 dependencies, wants vector tiles) remains rejected: vector tiles are a larger architectural change and a murkier ODbL position (D-049.7). The map remains fully usable with no tiles configured — the bundled Natural Earth outlines render as a Leaflet vector layer, and everything D-047.1 promised about offline deployments still holds.

**D-049.6 — Place search ships all 7,342 Natural Earth populated places, not the 4,215-place cut at 50k population.** The original complaint — "nobody can find their street corner at zoom 4" — is a navigation problem before it is an imagery problem, and it must have an offline answer because imagery only exists where an operator configured it. The dataset is Natural Earth populated places, release v5.1.2 pinned — the same public-domain source and release as the boundaries already bundled — reduced to name, country code, coordinates and population by `tools/build_datasets.py` with the source checksum recorded. The 50k cut was measured at 88 KB against 149 KB for the full set: 61 KB buys the 3,127 *smallest* listed places, and a cut at 50k discards precisely the small municipalities — the D-048.3 lesson (the 1:110m layer that omitted every small country) applied to towns. Search is a backend endpoint over the bundled index; **selecting a result moves the map and fills in no answer** — navigation is not autofill, and the three-input boundary of D-047 is not moved by this release in either direction.

**D-049.7 — No OSM-derived data is bundled, and the ODbL position is taken from the licence text, not a summary.** The package contains no OpenStreetMap data in any form: the bundled boundaries and places are Natural Earth (public domain), the climate grid is Beck et al. (CC BY 4.0). What an operator or user may configure is a *raster tile* source, typically OSM-derived. Under ODbL §4.4.7, using the database to create a Produced Work "does not create a Derivative Database" — share-alike does not reach it — and the OSMF Produced Work guideline lists rendered raster images as the canonical Produced Work. The obligation that does attach is ODbL §4.3's notice requirement on public use, which is why D-049.2 makes the attribution string a required half of the configuration and the map renders it whenever the layer is on. Vector tiles are arguably a Derivative Database (data intended for extraction rather than an image), which is one more reason D-049.5 stays with raster tiles and does not adopt a vector-tile stack. Nothing ODbL-licensed enters the wheel.

**D-049.8 — The request gate is replaced by something stronger, not deleted.** The static check (D-048.6's six shapes over both published artefacts) remains and remains valid — the deployer's tile URL is runtime state and never appears in build output, so a literal third-party URL in the build is still exactly the defect it always was. Added to it is a **runtime** check: the built app is driven headlessly (Playwright driving the installed system Chrome, the technique the v2.1 review used), first with no tile configuration through a complete assessment *including* the map, asserting zero requests leave the origin; then with a tile source configured — pointing at a second local port standing in for the third party, so CI itself stays offline — asserting tile requests go to that host and nowhere else, and that the map credits the source on screen; then with a configured but unreachable source, asserting the map degrades to the bundled outlines and the assessment remains completable. The degradation case is a promise D-049.1 makes to restricted-network deployments that have been configured optimistically, and it is verified rather than assumed.

**Out of scope, restated because imagery makes the temptation stronger:** autofill remains exactly the three inputs of D-047 (`site_area_m2`, `country`, `climate_zone`). Deriving canopy, imperviousness, LST or land use from what the imagery shows remains the GIS workflow deferred by D-002 and refused by D-047; neither the tile layer nor place search is a back door to it.

**No methodology version bump.** The place-search index changes no scoring value and enters no formula — it is navigation data, not a methodology value, and this is stated deliberately rather than bumping out of habit (the D-048.7 discipline). The methodology stays at `2026.08.05`; the package moves to 2.2.0.

## D-050 — v2.2 implementation rulings (2026-08-02)

Recorded during the implementation of D-049. None alters a methodology value, an engine formula, or a stored shape — the storage schema stays at 3, and the sensitivity analysis was re-run and is byte-identical to the published file, the D-048.7 treatment.

**D-050.1 — Tile failure means "nothing ever loaded", not "a tile failed".** The degradation D-049.8 promises fires after four tile-error events with zero successful loads; once any tile has loaded, later errors never degrade, because missing tiles at the edge of a provider's coverage are normal and yanking a working map over them would be a cure worse than the disease. The failure is remembered per template — switching source, toggling the deployer imagery, or pressing the retry affordance clears it — so an intermittent network can always try again. Verified in three places: the jsdom suite delivers error events to the tile images by hand, and the runtime gate's phase C drives the real browser against a dead port.

**D-050.2 — The user's source outranks the deployer's; a remembered failure outranks both.** The resolution order the interface implements for D-049.4 is: a user-named source, else the deployer's unless switched off for the visit, else nothing; and a source whose template previously failed outright resolves to nothing until retried. All of it is per-visit React state — nothing about tiles is ever persisted, which keeps the privacy sentence in the disclosure text ("the setting is not saved") literally true in both the user and deployer cases.

**D-050.3 — Place search ranks by scan order over a population-sorted index.** The bundled index is sorted population-descending at build time, so the backend answers "prefix matches first, then substring matches, each in index order" with no sort at query time — Paris, France precedes Paris, Kiribati and York precedes New York without a tiebreak table. Matching is case-insensitive over the name and its ASCII fold (stored only where it differs), so "malaga" finds Málaga; results are capped at ten and queries under two characters answer nothing. Coordinates are rounded to two decimals (~1.1 km) — the map is moved to a town, not a parcel — and selection sets only the map view, at zoom 12.

**D-050.4 — The runtime gate's unconfigured walkthrough goes *through* the map, deviating from the scope's letter deliberately.** The scope asked for an assessment completed "without opening the map"; since v2.1 the map is the wizard's first step, so a path that never renders it does not exist for a real user. The gate therefore does the stronger thing: it searches a place, draws a boundary, applies the autofill, completes and evaluates the assessment — and asserts zero external requests over all of it, map included. The same walkthrough incidentally re-verified an existing guard from the outside: a triangle drawn at world zoom is refused as a site area (`MAX_REASONABLE_SITE_AREA_M2`), which is why the scripted user, like a real one, navigates to city zoom first.

**D-050.5 — The app factory takes the tile configuration as a parameter, read from the environment exactly once.** `create_app(tiles=…)` defaults to reading the two variables at creation (so a misconfiguration fails the process at startup, per D-049.2) and accepts an explicit value so the test suite can pin "unconfigured" regardless of the developer's shell — the API conftest passes `tiles=None`, and the runtime gate scrubs `NATURE_COOLING_*` from the environment it hands the server under test. The CLI validates the same function before importing uvicorn, so a broken flag dies with a one-line error rather than a startup traceback.

**D-050.6 — Contrast is raised by pairing hue with value, still entirely from the tokens.** Land is `color-mix(in srgb, var(--accent) 20%, var(--paper-raised))` edged with `--ink-soft`; the sea is `--paper`, one step below the raised paper the land sits on. v2.1's pairing (`#dfe2d6` on `#f3f4ec`) differed in value only, and barely. With imagery on, the land fill would hide the tiles, so a `sitemap--imagery` state renders the same layer as outlines only — which is also what makes the D-049.8 degradation visible: the fill comes back when the tiles go.

**D-050.7 — Leaflet's controls are built explicitly, not by defaults.** The zoom control takes its titles from the message catalog (D-030's externalisation rule extends over library-rendered strings), and the attribution control is created with no prefix, so the only text it ever shows is the credits of layers actually on the map: the Natural Earth line for the bundled outlines, plus the configured source's line while tiles are on — which is the v2.1 attribution defect fixed in the direction of "the map credits what it shows", not just "the form stores a string".
