...
Four site conditions modulate archetype performance. Each condition resolves to
...
than skipping the question. The cap was introduced at version 2026.08.02 to
remove an incentive defect found during engine implementation: without it, a
site that declared reliable irrigation but left soil unknown could score below
one that answered neither, which would have taught users that the honest
answer costs them. Complete favourable data must strictly dominate silence. \\
\addlinespace
Scale &
...
\addlinespace
Climate &
A lookup of archetype family against climate zone
...
level assigned to each archetype family in each supported climate zone, resolved
to a factor through the condition-level table above. Vegetation-based
archetypes are downgraded in arid and semi-arid zones where water limitation
constrains evapotranspiration; blue-green archetypes are downgraded in
...
The Cooling Potential Score scales the base cooling score of the archetype the
selected entry inherits by the site adjustment factor:
...
where $B$ is the base cooling score from \Cref{tab:typologies}. Every
quantity in this subsection is a property of the \emph{archetype}, not of the
catalogue entry, and the result names the archetype it was computed against
(\Cref{sec:archetypes}).
...
where $[\tau_{\min}, \tau_{\max}]$ is the archetype's literature envelope from
\Cref{tab:typologies}.
...
\begin{figure}[htbp]
\centering
\input{figures/envelope-clipping}
...
\subsection{Multi-intervention packages}
\label{sec:packages}

An assessment may propose several simultaneously available catalogue entries.
Each component is \textbf{scored individually by the formulas above and
reported individually}; only the package's headline outputs are combined, and
they are combined under rules that add nothing the evidence does not support.
No rule below computes a new methodology value: each either selects among the
component results or takes an extremum of them.

\begin{table}[H]
\centering
\caption[Package combination rules]{How a package's headline outputs are
combined from its components. Every rule is a selection or an extremum; none
introduces a value that is not already a component result.}
\label{tab:package-rules}
\small
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{Output} & \textbf{Rule, and why} \\
\midrule
Temperature &
\textbf{The best-evidenced component's adjusted range; never summed.} No
retrieved source quantifies super-additive cooling from combining measures
(\Cref{sec:super-additivity}). Note that
$\min\bigl(\max_i \dT_{\max,i},\, \dT_{\max,\mathrm{rep}}\bigr)$ and
$\dT_{\max,\mathrm{rep}}$ are the same number, so the cap and the selection are
one rule, not two. \\
\addlinespace
Co-benefits &
\textbf{Union: the maximum per dimension}, then re-weighted by
\Cref{eq:cobenefit}. A package delivers each dimension at least as well as its
best component does, and this is where a package's real advantage lies. \\
\addlinespace
Suitability &
\textbf{The minimum across components.} A package is no more deliverable on a
site than its least suitable member; averaging would let a well-fitting
component conceal one that cannot be built here, which is exactly what the
flags of \Cref{sec:suitability} exist to prevent. \\
\addlinespace
Energy &
Derived from the best-evidenced \textbf{building-energy-applicable} component.
Energy is a function of one cooling estimate (\Cref{eq:energy}); restricting
rather than summing keeps the derivation traceable to exactly one component. If
no component is building-energy applicable, the energy status is
\texttt{typology\_not\_applicable}. \\
\addlinespace
Capital cost &
\textbf{Entered once for the whole package.} The tool ships no unit costs
(\Cref{sec:costs}), so there is nothing to apportion; \Cref{tab:app-fields}
states that the figure covers every selected component. \\
\addlinespace
Flags &
Per component, all surfaced, each naming its entry. The user must be able to
see \emph{which} part of the package is the problem. \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Which component speaks for the package.} The component carrying the
temperature is chosen by \textbf{evidence rating first, adjusted envelope width
second}, with ties falling to the user's own selection order so that the choice
is deterministic. A low-evidence component with a higher ceiling therefore does
not speak for the package --- otherwise the least certain estimate in a
selection would set its headline number.

\begin{keynote}
\noindent\textbf{Package size is unbounded, but warned above five components.}

Under the capped-never-summed rule a large package's headline temperature is
carried by its strongest member while the remainder add co-benefit breadth and
capital cost, so a hard cap would forbid something harmless. Above five
components the tool states plainly that adding a further component will not
raise the temperature estimate. That is the information a refusal would have
conveyed only by refusing.
\end{keynote}

\paragraph{A worked instance from the regression suite.} Golden scenario
\texttt{s23} selects five components on one temperate neighbourhood site: a
tree avenue (\SIrange{0.5}{3.0}{\degreeCelsius}, high evidence), a green median
(\SIrange{0.0}{1.5}{\degreeCelsius}, medium), a blue-green corridor
(\SIrange{1.0}{3.0}{\degreeCelsius}, medium), a pocket park
(\SIrange{0.5}{2.0}{\degreeCelsius}, high), and courtyard greening
(\SIrange{0.3}{1.5}{\degreeCelsius}, low). The reported range is the tree
avenue's alone --- \SIrange{0.5}{3.0}{\degreeCelsius} at an adjustment factor of
\num{1.0} --- because it is the best-evidenced component and, among the two
high-evidence components, the one with the wider envelope. The sum of the five
ceilings would be \qty{11.0}{\degreeCelsius}, and the tool does not claim it.
The co-benefit score, unioned, is \num{71.25}; the suitability score is the
weakest component's \num{86.25}; and the Opportunity Score is \num{76.10}. The
scenario exists to make a regression in any one of these rules fail the build.
...
The co-benefit sub-indicators take per-archetype default levels, each cited in
the library (\Cref{app:typologies}), which the user may override. In a package
they are unioned across components before weighting (\Cref{tab:package-rules}).

\paragraph{The sub-indicator rules, fixed at version 2026.08.01.} An earlier
...
sub-indicators. A score of 25 on space, soil, or water
...
\midrule
Space &
$r = \text{site area} / \text{minimum viable area}$:
$r < 1$ $\rightarrow$ 25 \emph{and the flag}; $1 \le r < 2$ $\rightarrow$ 50;
$2 \le r < 5$ $\rightarrow$ 75; $r \ge 5$ $\rightarrow$ 100. \\
\addlinespace
Soil &
Availability against the entry's requirement on the ordinal scale
...
\addlinespace
Urban context &
Site land use inside the entry's own land-use list $\rightarrow$ 100;
...
weights include a \emph{social inclusion} term for which the library
...
the archetype the selected entry inherits is flagged as capable of affecting
adjacent building loads. In a package the derivation follows one
building-energy-applicable component (\Cref{tab:package-rules}).
Otherwise the result carries an explicit status, checked in a fixed order ---
...
unit costs for the interventions in this library.
...
feasibility. The bracket boundaries and combination rule were fixed at version
2026.08.01, and are declared rather than derived: no source in the
bibliography establishes a payback threshold at which an urban cooling
investment becomes attractive, because that judgement belongs to the
institution spending the money. What the methodology owes is that the
thresholds be stated, versioned, and adjustable in configuration rather than
buried in code. They are as follows. Because the energy saving is an
...
to be noticed afterwards. It follows from a deliberate value stance taken when
the instrument was designed: that the tool should prioritise who is affected
slightly above how hot a place is. The alternative designs --- dropping
